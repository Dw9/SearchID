...
my_ipv4_address_to_string(char* buffer, size_t capacity, const struct sockaddr_in* addr,
                       size_t addrlen) {
...
static mdns_string_t
my_ipv6_address_to_string(char* buffer, size_t capacity, const struct sockaddr_in6* addr,
                       size_t addrlen) {
    char host[NI_MAXHOST] = {0};
    char service[NI_MAXSERV] = {0};
    int ret = getnameinfo((const struct sockaddr*)addr, addrlen, host, NI_MAXHOST, service,
...
static mdns_string_t
my_ip_address_to_string(char* buffer, size_t capacity, const struct sockaddr* addr, size_t addrlen) {
    if (addr->sa_family == AF_INET6)
        return my_ipv6_address_to_string(buffer, capacity, (const struct sockaddr_in6*)addr, addrlen);
    return my_ipv4_address_to_string(buffer, capacity, (const struct sockaddr_in*)addr, addrlen);
}
...
//run function
int run_service_mdns(char*);

void release_mdns_data(void);
...
mdns_string_make_ref(void* data, size_t capacity, size_t ref_offset) {
    ...
    char* udata = (char*)data;
    //uint16_t* udata = (uint16_t*)data;

    //*udata++ = htons(0xC000 | (uint16_t)ref_offset);

    // cdata[0] = 0xc0 | (ref_offset >> 8);
    // cdata[1] = ref_offset & 0xff;
    *udata++ =  0xc0 | (ref_offset >> 8);;
    *udata++ = ref_offset & 0xff;
    return udata;
}
...
mdns_multicast_send(int sock, const void* buffer, size_t size) {
    ...
    // char buftemp[256] = "";
    // if (sendto(sock, (const char*)buftemp, 163, 0, saddr, saddrlen) < 0)
    //      return -1;
  //  printf("send\n");
...
mdns_socket_listen(int sock, void* buffer, size_t capacity, mdns_record_callback_fn callback,
    ...
    mdns_string_t ipadd = my_ip_address_to_string(myaddrbuffer, sizeof(myaddrbuffer), saddr, addrlen);
    //printf("recv ip:  %.*s \n", MDNS_STRING_FORMAT(ipadd));
    // const char* dstIpaddr = "192.168.1.104:5353";
    // int ret0 = strcmp(dstIpaddr,ipadd.str);
    // if(ret0 != 0){
    //     //printf("recv ipaddr: %s\n",ipadd.str);
    //     return 0;
    // }
    //printf("recv\n");
    //for (int i = 0; i < ret; i ++){
    ...
    uint16_t* data = (uint16_t*)buffer;
    /* 头部总共12字节 */
//     printf("recv buffer is: \n");
//     char* showbuf = (char*)malloc(sizeof(char) * 40);
//     memcpy(showbuf,(char*)(data+6),40);
//     printf("%s\n",showbuf);
    uint16_t transaction_id = ntohs(*data++);  //每个标注位2字节
    uint16_t flags = ntohs(*data++);
    uint16_t questions = ntohs(*data++);
    //printf("data questions************%x\n",*(data -1));
    /*
    ...
    */
    ...
    for (int iquestion = 0; iquestion < questions; ++iquestion) {
        ...
        //printf("question_offset %d\n",question_offset);
        size_t offset = question_offset;
        size_t verify_ofs = 12;
        // printf("question_offset: %ld\n",question_offset);
        ...
        // printf("after fun\n");
        // printf("question_offset: %ld",question_offset);
        size_t length = offset - question_offset;
        // printf("skip byte size_t************%zu\n",sizeof(size_t));
        // printf("skip byte length************%zu\n",length);
        // printf("skip byte offset************%zu\n",offset);
        // printf("skip byte question_offset************%zu\n",question_offset);
        // printf("buffer[%d] = \\x%02x\n", offset, ((char*)buffer)[offset]);
        // printf("buffer[%d] = \\x%02x\n", offset+1, ((char*)buffer)[offset+1]);
        // printf("buffer[%d] = \\x%02x\n", offset+2, ((char*)buffer)[offset+2]);
        // printf("buffer[%d] = \\x%02x\n", offset+3, ((char*)buffer)[offset+3]);

        char* data_ptr = (char*)buffer + offset; 

        // int distance = (char*)data - (char*)buffer;
        // printf("data x************%d\n",distance);
        // printf("data 0************%u\n",*data);
        // printf("data 1************%u\n",*(data+1));
        // printf("data 2************%u\n",*(data+2));
        ...
        uint16_t rtype = data_ptr[0] << 8 | data_ptr[1];
        uint16_t rclass = data_ptr[2] << 8 | data_ptr[3];
        // uint16_t rtype =(uint16_t) *data;
        // uint16_t rclass =(uint16_t) (*(data+1));
        //printf("uint16_t rtype************%d\n",rtype);
        //printf("uint16_t rclass************%d\n",rclass);    

        // Make sure we get a PTR question of class IN
        // if(rtype ==0 && rclass == 12){
        //     printf("hello world\n");
        //     *data = 0X0c00;
        //     *(data+1) = 0X0100;
        //     rtype = 12;
        //     rclass = 1;
        //     printf("uint16_t rtype  again************%d\n",rtype);
        //     printf("uint16_t rclass again************%d\n",rclass);  
        // }
        if ((rtype != MDNS_RECORDTYPE_PTR) || ((rclass & 0x7FFF) != MDNS_CLASS_IN)){
            //printf("**********whatever**********\n"); 
            return 0;
...
mdns_query_answer(int sock, const void* address, size_t address_size, void* buffer, size_t capacity,
    ...
    header->answer_rrs = htons(5);
    header->authority_rrs = htons(0);
    header->additional_rrs = htons(0);

    // header->transaction_id = htons(transaction_id);
    // header->flags = 0x00 << 8 | 0x84;
    // header->questions = htons(0);
    // header->answer_rrs = 0x05 <<8 | 0x00;
    // header->authority_rrs = 0;
    // header->additional_rrs = htons(0);
    ...
    void* data = MDNS_POINTER_OFFSET(buffer, sizeof(struct mdns_header_t));
    size_t service_offset = MDNS_POINTER_DIFF(data, buffer);
    //printf("MDNS_POINTER_DIFF:service_offset: %d\n ",service_offset);
    size_t remain = capacity - service_offset;
    data = mdns_string_make(data, remain, service, service_length);
    //size_t local_offset = MDNS_POINTER_DIFF(data, buffer) - 7;
...
//length
    char* record_length = ydata; // length指针记录
    *ydata++ = 0x00; 
    *ydata++ = 0x0d; 
    ...
    if (!data || (remain <= 10))
        return -1;
    *record_length = htons((uint16_t)MDNS_POINTER_DIFF(data, record_length + 2));
    ...
    ydata = (char*)data;
    *ydata++ = 0x00 ;  // MDNS_RECORDTYPE_SRV
    *ydata++ = 0x21;   
    *ydata++ = 0x00 ;  // rclass 1
    *ydata++ = 0x01; 
    //ttl 
    ...
    *ydata++ = 0x0a;  
    ...
    //length
    record_length = (char*)ydata;  // 记录长度位置 
    *ydata++ = 0x00; 
    *ydata++ = 0x13;     
    ...
    *record_length = htons((uint16_t)MDNS_POINTER_DIFF(data, record_length + 2));
    ...
    if (use_ipv4) {
        ...
        *ydata++ = 0x04;       // length
        ...
        // *(uint32_t*)ydata = ipv4;  // ipv4 address
        // ydata += 4;
    ...
    if (use_ipv6) {
        ...
        *ydata++ = 0x0a;    
        ...
        // *udata++ = 0x1c << 8 || 0x00;  // type
        // *udata++ = 0x01 <<8  || 0x00;         // rclass
        // *(uint32_t*)udata = 0x0A << 8 || 0x00;           // ttl
        //udata += 2;
        //*udata++ = htons(16);     // length

        //*udata++ = 0x10 << 8 | 0x00;     // length
        *ydata++ = 0x00; 
        *ydata++ = 0x10;    

        memcpy(ydata, ipv6, 16);  // ipv6 address

        data = MDNS_POINTER_OFFSET(ydata, 16);
        remain = capacity - MDNS_POINTER_DIFF(data, buffer);
    }
