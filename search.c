...
const char* id_service = "_test_mdns._tcp.local.";
int id_service_port = 42424;
...
int checkNC(char* NetCardUser,const char* NetCrad){
    return strcmp(NetCardUser,NetCrad);
}
...
service_callback(int sock, const struct sockaddr* from, size_t addrlen, mdns_entry_type_t entry,
    ...
    if (entry != MDNS_ENTRYTYPE_QUESTION){
        ...
        //printf("entry != MDNS_ENTRYTYPE_QUESTION\n");
        return 0;
    }
...
send_dns_sd(void) {
	int sockets[32];
	num_sockets = open_client_sockets(sockets, sizeof(sockets) / sizeof(sockets[0]));
	if (num_sockets <= 0) {
...
service_mdns(const char* hostname, const char* service, int service_port) {
	...
	num_sockets = open_service_sockets(sockets, sizeof(sockets) / sizeof(sockets[0]));
	if (num_sockets <= 0) {
		printf("Failed to open any client sockets\n");
		return -1;
	}
...
//
//                    int ret = recvfrom(sockets[isock], (char*)buffer, (mdns_size_t)capacity, 0, saddr, &addrlen);
//                    if (ret <= 0)
//                        return 0;
//
//                    mdns_string_t fromaddrstr = ip_address_to_string(addrbuffer, sizeof(addrbuffer), saddr, addrlen);
//                    printf("recv ip:  %.*s \n", MDNS_STRING_FORMAT(fromaddrstr));
...
int run_service_mdns(char* uuid_string) {

	service_mdns(hostname, id_service, id_service_port);
	return 0;
}

int main(){

    run_service_mdns("fuck");
    return 0;
}